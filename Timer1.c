...
//FUNCTION DEFINITIONS HERE: -----------------------------------------------------------------------
	...
	void timer1_init(float curr_freq)   //16 bit timer
	{
		...
		val = (uint32_t)FOSC/((uint32_t)PRESCALER_METRICS);
		val = val/1000000;   
		
		OCR1A = (uint16_t) val;   //2 counts per micro-second

		printf("The CTC value is: %u\n",OCR1A);
	...
	void start_timer1()
	{
		...
		if(PRESCALER_METRICS == 1)
		{
			TCCR1B |= (1 << CS10) ;
		}
		if(PRESCALER_METRICS == 8)
		{
			TCCR1B |= (1 << CS11);
		}
		if(PRESCALER_METRICS == 64)
		{
			TCCR1B |= (1 << CS11) | (1 << CS10);
		}
		if(PRESCALER_METRICS == 256)
		{
			TCCR1B |= (1 << CS12);
		}
		if(PRESCALER_METRICS == 1024)
		{
			TCCR1B |= (1 << CS12) | (1 << CS10);
		}
